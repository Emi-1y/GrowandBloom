...
<div class="row g-5 align-items-start" style="margin-top:1rem;">
        ...
        <div style="border-radius:16px; overflow:hidden; background:#f2efe8; aspect-ratio:4/3; display:flex; align-items:center; justify-content:center; border:1px solid #e2ddd4;">
            ...
            @else
                <div style="text-align:center; color:#7a7165; opacity:.4;">
                    ...
                    <div style="font-size:.75rem; letter-spacing:.1em; text-transform:uppercase;">{{ __('product.no_image') }}</div>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="col-md-6">
        ...
        <div style="font-size:1.6rem; font-weight:700; color:#2d5a3d; margin-bottom:1.5rem;">
            ...
            <small style="font-size:.75rem; color:#7a7165; font-weight:400;">{{ __('product.currency') }}</small>
        ...
        <div style="border:1px solid #e2ddd4; border-radius:12px; overflow:hidden; margin-bottom:1.5rem;">
            <div style="display:grid; grid-template-columns:1fr 1fr;">
                <div style="padding:.875rem 1.25rem; border-right:1px solid #e2ddd4; border-bottom:1px solid #e2ddd4;">
                    <div style="font-size:.65rem; letter-spacing:.12em; text-transform:uppercase; color:#7a7165; font-weight:600; margin-bottom:3px;">{{ __('product.label_variety') }}</div>
                ...
                <div style="padding:.875rem 1.25rem; border-bottom:1px solid #e2ddd4;">
                    <div style="font-size:.65rem; letter-spacing:.12em; text-transform:uppercase; color:#7a7165; font-weight:600; margin-bottom:3px;">{{ __('product.label_size') }}</div>
                ...
                <div style="padding:.875rem 1.25rem; border-right:1px solid #e2ddd4;">
                    <div style="font-size:.65rem; letter-spacing:.12em; text-transform:uppercase; color:#7a7165; font-weight:600; margin-bottom:3px;">{{ __('product.label_availability') }}</div>
                    <div style="font-size:.9rem;">
                        @if($viewData['product']->getStock() > 0)
                            <span style="color:#2d5a3d; font-weight:600;">{{ $viewData['product']->getStock() }} {{ __('product.label_available') }}</span>
                        @else
                            <span style="color:#7a7165;">{{ __('product.label_out_of_stock') }}</span>
                        @endif
                    </div>
                </div>
                <div style="padding:.875rem 1.25rem;">
                    <div style="font-size:.65rem; letter-spacing:.12em; text-transform:uppercase; color:#7a7165; font-weight:600; margin-bottom:3px;">{{ __('product.label_category') }}</div>
